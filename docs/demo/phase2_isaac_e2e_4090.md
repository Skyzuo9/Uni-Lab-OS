...
## 2A. Managed 模式启动 Edge + Isaac Worker

方案 A 的 managed 模式由 Uni-Lab-OS edge 进程托管 Isaac worker 生命周期。它不会自动从
registry/graph 生成 USD stage；本阶段仍使用显式的 `--physics_scene` LabUtopia USD。

如果 `8092` 已有旧 worker，先找到精确 PID 再停止：

```bash
ssh ubuntu@172.20.0.39 'pgrep -af "[u]nilabos.sim.backends.isaac.worker.*8092"'
```

用一条 edge 命令启动 managed worker、Query gRPC 和 FastAPI：
...
ssh ubuntu@172.20.0.39 '
  ...
  nohup /home/ubuntu/miniforge3/bin/conda run --no-capture-output -n unilab env PYTHONPATH=. \
    unilab --graph unilabos/test/experiments/mock_devices/mock_all.json \
      ...
      --isaac_python python \
      --isaac_log_path /tmp/isaac_worker_managed_8092.log \
      --isaac_start_timeout 300 \
      --isaac_camera /World/Camera \
      --isaac_rpc_timeout_s 600 \
      ...
      --test_mode \
      --port 8002 \
    > /tmp/unilab_edge_managed_c5_50052.log 2>&1 &
  echo $! > /tmp/unilab_edge_managed_c5_50052.pid
'
```

检查 managed worker 和 edge：

```bash
ssh ubuntu@172.20.0.39 '
  sleep 90
  ss -ltnp "( sport = :8092 or sport = :50052 or sport = :8002 )"
  curl -sS http://127.0.0.1:8092/health
  tail -n 120 /tmp/unilab_edge_managed_c5_50052.log
'
```

如果 managed 模式失败，保留下面的手动 worker + edge 分步流程作为 fallback。
