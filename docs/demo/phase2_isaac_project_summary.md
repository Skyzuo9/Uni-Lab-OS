...
- CLI 新增：
  ...
  - `--physics_scene`
  - `--physics_timeout`
- CLI 新增 managed Isaac worker 选项：
  - `--isaac_managed`
  - `--isaac_host` / `--isaac_port`
  - `--isaac_conda_env` / `--isaac_conda_executable` / `--isaac_python`
  - `--isaac_log_path` / `--isaac_start_timeout`
  - `--isaac_headless` / `--no_isaac_headless`
  - `--isaac_camera` / `--isaac_rpc_timeout_s`
- Backend 启动时自动构建 physics backend，并写入 `RuntimeContext`。
- 如果启用 `--isaac_managed`，edge 会先启动 Isaac worker、等待 `/health`，再注入 `physics_endpoint` 并连接 `IsaacBridgeBackend`。
...
- `unilabos/sim/backends/isaac/managed.py`
