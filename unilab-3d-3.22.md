...
* 注：unilab-assets里面大部分仪器os都尚未接管，有的可能也不用接管。需要拓展已有注册表或者针对未托管的仪器增加新的表
...
* 初步判断pencil不太适合做layout calculator，它还是比较注重可视化方面。可能有一些方法可以复用作为朝向、距离等微调。
...
### 8.4 AI自动排布
需注意：要添加两个高级约束：
* **生化环境约束**：细化如防交叉污染流向（PCR实验室的试剂准备区、标本制备区需要物理隔离排序）、震动隔离（高精度天平 mettler_toledo_xpr 不能与大型离心机靠得太近）等实际生化实验室独有的规则。（对应增加硬编码规则）
* **运动学可达性空间约束**：两个设备（如自动移液工作站与离心机）如果需要依靠复合机器人（AGV+Arm）或滑轨机械臂传递样本，AI排布时必须确保两个设备的交接点（Transfer Port）都落在机械臂的运动学工作空间内。
...
## 附录资料
* 现有模型分类库：
* [Matterix_Asset_Classification.md]