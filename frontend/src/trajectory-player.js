import { updateJointState } from './device-renderer.js';
...
export class TrajectoryPlayer {
  ...
  loadTrajectory(displayTrajectory) {
    ...
    this.trajectory = {
      ...
      points: jt.points.map(pt => ({
        ...
        timeFromStart: (pt.time_from_start?.sec || 0) + (pt.time_from_start?.nanosec || 0) * 1e-9,
      })),
    };
  ...
  _applyFrame(idx) {
    updateJointState({ name: this.trajectory.jointNames, position: this.trajectory.points[idx].positions });
  }
  ...
  _buildUI(container) {
    ...
    div.innerHTML = '<span style="color:#4fc3f7;font-weight:bold;">Trajectory Preview</span> '
      ...
      + '<select id="traj-speed"><option value="0.25">0.25x</option><option value="0.5">0.5x</option><option value="1" selected>1x</option><option value="2">2x</option></select> '
