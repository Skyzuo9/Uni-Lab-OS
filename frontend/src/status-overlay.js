...
const STATUS_COLORS = {
  idle: 0x888888, running: 0x2196F3,
  completed: 0x4CAF50, error: 0xF44336, warning: 0xFF9800,
};
...
export class StatusOverlay {
  ...
  connectWebSocket(baseUrl = '') {
    ...
    this.ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'device_status') this._apply(msg.data.device_status);
      } catch (e) { }
    };
    this.ws.onclose = () => {
  ...
  setStatus(deviceId, status) { this._apply({ [deviceId]: status }); }
}
